pro find, image, x, y, flux, sharp, roundness, hmin, fwhm, roundlim, sharplim,$
                      PRINT = print, SILENT=silent, MONITOR= monitor,nfound=nfound, verbose=verbose
;+
; NAME:
;	FIND
; PURPOSE:
...
; positive then the centroid does not make sense 
  ...
  if (hx LE 0) then begin
      badcntrd = badcntrd + 1
      If keyword_set(verbose) then print,'Bad hx : ',hx
      nfound+=-1 
	 goto, REJECT
  endif
 ...
 if abs(dx) GE nhalf then begin 
     badcntrd = badcntrd + 1
     nfound+=-1
      If keyword_set(verbose) then print,'Bad dx : ',dx,nhalf
	 goto, REJECT
  endif
...
; Find Y centroid                 
 ...
 hy = (sumgd - sumgy*sumd/p) / (sumgsqx - sumgy^2/p)
  ...
  if (hy LE 0) then begin
     badcntrd = badcntrd + 1
     nfound+=-1
     If keyword_set(verbose) then print,'Bad hy : ',hy
	 goto, REJECT
  endif
 ...
 if abs(dy) GE nhalf then begin
        If keyword_set(verbose) then print,'Bad dy : ',dy,nhalf
	badcntrd = badcntrd + 1
	 goto, REJECT
  endif
